...
class Controller:
    ...
    def run_and_return(self):
        ...
        while self.not_finished():
            ...
            if key == ord('q') or key == 27: # Q key of Esc to quit the program
                break
            elif key == ord('s'):  # S key to return drawn image
                # self.save()
                # self.img, self.current_class = self.set_new_image()
                if len(self.designers.flatten()) != 1:
                    print("Designers need to have size 1 to return 1 output image!")
                    break
